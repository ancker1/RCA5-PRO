...
int main(int _argc, char **_argv) {
    ...
    //    Mat small_map = cv::imread( "../map_control/floor_plan.png", IMREAD_COLOR );
        cv::Mat small_map = cv::imread( "../models/smallworld/meshes/floor_plan.png", cv::IMREAD_COLOR );
        cv::resize(small_map, small_map, small_map.size()*2,0,0,cv::INTER_NEAREST);
        cv::Mat mapclone = small_map.clone();
        mapclone.at<cv::Vec3b>(startii) = cv::Vec3b(0, 255, 0);
        mapclone.at<cv::Vec3b>(goalii) = cv::Vec3b(0, 0, 255);
        cv::resize(mapclone, mapclone, mapclone.size()*10,0,0,cv::INTER_NEAREST);
        cv::imshow("Tekst",mapclone);
